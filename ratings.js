...
// ------------------------------------------------------------JacRed------------------------------------------------------------------------------    
    function getBestReleaseFromJacred(normalizedCard, localCurrentCard, callback) {
        ...
        function searchJacred(searchTitle, searchYear, exact, stepName, callback) {
            ...
            new Lampa.Reguest().silent(apiUrl, function(response) {
                ...
                try {
                    ...
                    // Функция извлечения качества из названия (ES5)
                    ...
                    function extractYear(title) {
                        ...
                        // Ищем все вхождения года в строке
                        while ((match = regex.exec(title)) !== null) {
                            var year = parseInt(match[1], 10);
                            
                            // Проверяем валидность года
                            if (year >= 1900 && year <= currentYear + 1) {
                                lastYear = year; // Запоминаем последнее валидное вхождение
                            }
                        }
                    ...
                    for (var i = 0; i < torrents.length; i++) {
                        ...
                        // Обработка качества
                        if (typeof t.quality !== 'number') continue;
                        if (t.quality === 0) {
                        ...
                        // Обработка года для каждого торрента
                        var yearValid = false;
                        var parsedYear = 0;
                        
                        // Пытаемся получить год из поля relased
                        if (usedYear && !isNaN(usedYear) && usedYear > 1900) {
                            parsedYear = parseInt(usedYear, 10);
                            yearValid = true;
                        }
                        
                        // Если поле relased не содержит валидный год, парсим из названия
                        if (!yearValid) {
                        ...
                        if (yearValid) {
                            if (!isNaN(searchYearNum)) {
                                //if (Math.abs(parsedYear - searchYearNum) <= 1) {
                        ...
                        if (Q_LOGGING) {
                            console.log(
                                ...
                                " | Y: " + (yearValid ? parsedYear : "unknown") + " (" + yearSource + ")" +
                                " | Status: " + yearStatus +
                                " | Step: " + stepName
                            );
                        }
        ...
        // Стратегия 2: По title (точное совпадение)
        ...
        // Стратегия 3: По original_title (точное совпадение) -1год
        /*
        if (normalizedCard.original_title && (hasLetters(normalizedCard.original_title) || onlyDigits(normalizedCard.original_title))) {
        ...
        */
        ...
        /*
        // Стратегия 5: По title (без точного совпадения)
        if (normalizedCard.title && hasLetters(normalizedCard.title)) {
            searchStrategies.push({
                title: normalizedCard.title.trim(),
                exact: false,
                name: "Title NonExact"
...
// ------------------------------------------------------------END--JacRed-------------------------------------------------------------------------     
    ...
    function fetchAdditionalRatings(card, render) {
        ...
        function updateUI() {
            ...
            if (parseInt(localStorage.getItem('maxsm_ratings_mode'), 10) !== 2)
    ...
    function calculateAverageRating(localCurrentCard, render) {
        ...
        var mode = localStorage.getItem('maxsm_ratings_mode');
        
        if (totalWeight > 0 && (ratingsCount > 1 || (parseInt(localStorage.getItem('maxsm_ratings_mode'), 10) === 1))) {
            ...
            if (parseInt(localStorage.getItem('maxsm_ratings_mode'), 10) === 1) {
